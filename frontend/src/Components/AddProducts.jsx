...
const AddProducts = (props) => {
  const { userId, setProducts } = useData();
  ...
  const handleSubmit = async (e) => {
    if (!input.name || !input.description || !input.manufacturer) {
      showToastMessage("error", "fill all input field");
      return;
    }
    ...
    try {
      const { data } = await addProduct(userId, newProduct);
      setProducts((prev) => {
            ...
            <form>
              ...
              <div class="flex items-center justify-center space-x-2 rounded-b border-t border-gray-200 p-6 ">
                <button
                  ...
                  type="submit"
                  disabled={
                    !input.name || !input.description || !input.manufacturer
                  }
